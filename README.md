...
*   Start the Twingate desktop client with `twingate start`.

*   Login to Claude Code with your API key.

*   Install Outlook and Teams as Progressive Web Apps through Chrome, if required.

*   Execute the `./bin/idea.sh` script from the extracted directory to run IntelliJ IDEA. Create a desktop entry through Settings.

*   Clone repositories (preferably with SSH keys) and build docker containers.
